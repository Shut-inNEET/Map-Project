...
public:
  ...
  // Ratio of the number of elements in the hash table to the number of buckets
  double getLoadFactor() const {
      double numElements = 0;
      for (const auto& bucket : table) {
          numElements += bucket.size();
      }
      double loadFactor = static_cast<double>(numElements) / table.size();
      return loadFactor;
  }

  // Calculates and returns the number collisions 
  // Iterates through each bucket and count the number of elements in each bucket that are greater than 1 
  // (since a collision occurs when multiple elements are hashed to the same bucket)
  ...
  // (i.e., the bucket with the maximum number of elements). 
  // We can do this by iterating through each bucket and keeping track of the maximum size encountered.
  int maxBucketSize() const {
      int largestBucket = 0;
      for (const auto& bucket : table) {
          largestBucket = std::max(largestBucket, static_cast<int>(bucket.size()));
      }
